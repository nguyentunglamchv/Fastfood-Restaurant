...
namespace Quanlyquanan
{
    public partial class FQuanLy : Form
    {
        ...
        void AddFoodBinding() //Hiển thị thông tin từ datagridview sang textbox
        {
            ...
            numGiaThucAn.Value.ToString("N0");
        ...
        void ShowBill(int id)   //Hiện hóa đơn các món của bàn ăn
        {
            ...
            txtTotalPrice.Text = totalPrice.ToString("N0");//ToString("c", culture); // Hiện tiền kiểu 0.000
        ...
        public void tinhTongDoanhThu() //Tính tổng doanh thu
        {
            try
            {
                ...
                txtTongDoanhThu.Text = thanhtien.ToString("N0");
               // CultureInfo culture = new CultureInfo("vi-VN"); // Ở chổ tính tiền sẽ hiện đơn vị tiền VN
        ...
        void AddAccountBinding(string textbox) //Hiển thị thông tin từ datagridview sang textbox ( dùng để truyền dữ liệu qua form đổi mật khẩu)
        {
            ...
            dtpk_ngaysinh.DataBindings.Add(new Binding("Value", dgvTaiKhoan.DataSource, "birthday", true, DataSourceUpdateMode.Never));
           txtMatKhauTK.DataBindings.Add(new Binding("Text", dgvTaiKhoan.DataSource, "PassWord", true, DataSourceUpdateMode.Never));
            guna2ComboBox1.DataBindings.Add(new Binding("Text", dgvTaiKhoan.DataSource, "gender", true, DataSourceUpdateMode.Never));//giotinh
            txtDiachi.DataBindings.Add(new Binding("Text", dgvTaiKhoan.DataSource, "address", true, DataSourceUpdateMode.Never));
        ...
        void AddAccount(string userName, string displayName, int type, string pass, string birth, string gen, string add) //Thêm tài khoản
        {
            if (AccountDAO.Instance.InsertAccount(userName, displayName, type,pass,birth,gen,add))
            {
                ...
                if (idBill != -1)
                {
                                                    ...
                                                    table.Name, totalPrice, discount, finalTotalPrice), "Thông báo", MessageBoxButtons.OKCancel) == System.Windows.Forms.DialogResult.OK)
                    {
                        ...
                        for (int i = 0; i < lvHoaDon.Items.Count; i++)
                        {
                            ...
                            tmp.tenmon = lvHoaDon.Items[i].SubItems[0].Text;
                            tmp.soluong = int.Parse(lvHoaDon.Items[i].SubItems[1].Text);
                            tmp.dongia = int.Parse(lvHoaDon.Items[i].SubItems[2].Text);
                            tmp.thanhtien = int.Parse(lvHoaDon.Items[i].SubItems[3].Text);
                            listBillDetail.Add(tmp);
                        }
                        LoadTable();

                        FExportBill frm = new FExportBill(table.Name, txtTotalPrice.Text);
                        rs.Name = "DataSet1";
                        rs.Value = listBillDetail;
                        frm.reportViewer1.LocalReport.DataSources.Clear();
                        frm.reportViewer1.LocalReport.DataSources.Add(rs);
                        frm.reportViewer1.LocalReport.ReportEmbeddedResource = "Quanlyquanan.Report1.rdlc";
        ...
        private void btThemTK_Click(object sender, EventArgs e)
        {
            ...
            string pass = txtMatKhauTK.Text;
            string birth = dtpk_ngaysinh.Text;
            string gen = guna2ComboBox1.Text;
            string add = txtDiachi.Text;
            AddAccount(userName, displayName, type,pass,birth,gen,add);
        }
